...
\[ \tau_{M_n(\mathbb{C})} = \{ f^{-1}(CU) \mid U \in \tau_{\mathbb{C}^{n^2}} \} \]
\end{remark}
...
where
\[
q: M_n(\mathbb{C}) \times M_n(\mathbb{C}) \xrightarrow{(\sigma, \tau)} \sigma \tau
\]
is matrix multiplication. And $\rho$ is a polynomial to $p((b_{ij}), (b'_{ij})) = (c_{ij})$ where
\[
c_{ij} = \sum_{k=1}^n b_{ik} b'_{kj}.
\]
Hence $\rho$ is continuous. Therefore for $q = \rho \circ (f \times f)$ is continuous. For any open set in $M_n(\mathbb{C})$ $U$, we express $U = f^{-1}(O)$ for some open set $O$ in
\end{proof}
...
is set of orthogonal matrices. If $\sigma = \sigma^*$ then $\sigma$ is complex orthogonal and if $\sigma = \sigma^* = \overline{\sigma}$ then real orthogonal.

$\mathrm{U}_n(\mathbb{C}) = \left\{ \sigma \in \mathrm{GL}_n(\mathbb{C}) \mid \sigma^* = \overline{\sigma}^{-1} \right\}$

is set of all unitary matrices.

Note $\mathrm{U}_n(\mathbb{C})$ or $\mathrm{O}_n(\mathbb{C})$ are non empty as $\mathbb{E} \in \mathrm{U}_n(\mathbb{C})$ and $\mathbb{E} \in \mathrm{O}_n(\mathbb{C})$.

Notation: $\mathrm{U}_n(\mathbb{C}) = (\mathrm{UC}_n)$ and $\mathrm{O}_n(\mathbb{R}) = \mathrm{OC}_n$. Don't be confused!
\end{definition}
...
The sets $U(n) = \{\sigma \mid \sigma = \sigma^* \}$ and $O_n(\mathbb{C}) = \{\sigma \mid \sigma = \sigma^* \overline{\sigma}^T\}$ are closed sets. Observe $GL_n(\mathbb{R}) = \{\sigma \mid \sigma = \overline{\sigma}\}$ is also closed. Hence $U(n)$ and
...
% --- Page 12 --- ERROR: HTTPSConnectionPool(host='api.mistral.ai', port=443): Max retries exceeded with url: /v1/chat/completions (Caused by NameResolutionError("HTTPSConnection(host='api.mistral.ai', port=443): Failed to resolve 'api.mistral.ai' ([Errno 11001] getaddrinfo failed)"))
