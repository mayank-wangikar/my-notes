...
\[ \tau_{M_n(\mathbb{C})} = \{ f^{-1}(CU) \mid U \in \tau_{\mathbb{C}^n} \times \tau_{\mathbb{C}^n} \} \]
\end{remark}
...
\begin{lemma}
\label{lemma:transpose_conjugate}
...
\[
\begin{CD}
M_n(\mathbb{C}) @>h>> M_n(\mathbb{C}) \\
@V{}V{A \mapsto ^t A}V @V{}V{A \mapsto \overline{A}}V
\end{CD}
\]
we can construct \( r: \mathbb{C}^{n^2} \to \mathbb{C}^{n^2} \) which
is a rearrangement of indices such that
\[
b_{i+j-n} = b_{j+i-c-1n} \quad \text{which is a homeomorphism.}
\]
...
\[
\begin{CD}
M_n(\mathbb{C}) @>k>> M_n(\mathbb{C}) \\
@V{}V{A \mapsto \overline{A}}V @V{}V{A \mapsto ^t A}V
\end{CD}
\]
\end{proof}
...
\]
\[
\frac{d(\alpha \beta)}{dt} = \frac{d\alpha}{dt} \cdot \beta + \alpha \cdot \frac{d\beta}{dt}
\]
...
\begin{remark}
$\mathrm{GL}_n(\mathbb{C}) = \{\sigma \in M_n(\mathbb{C}) \mid \det \sigma \neq 0 \}$
\[
...
\[
\begin{definition}
\text{O}_n(\mathbb{C}) = \left\{ \sigma \in \text{GL}_n(\mathbb{C}) \mid \sigma^* = \sigma^{-1} \right\}
\end{definition}
\]
...
\[
\begin{definition}
\text{U}_n(\mathbb{C}) = \left\{ \sigma \in \text{GL}_n(\mathbb{C}) \mid \sigma^* = \sigma^{-1} \right\}
\end{definition}
\]

is set of all unitary matrices.

Note $\text{U}_n(\mathbb{C})$ or $\text{O}_n(\mathbb{C})$ are non empty as $e \in \text{U}_n(\mathbb{C})$ and $e \in \text{O}_n(\mathbb{C})$.

Notation: $\text{U}_n(\mathbb{C}) = (\text{UC}_n)$ and $\text{O}_n(\mathbb{R}) = \text{OC}_n$. Don't be confused!
...
\begin{proof}
Consider \( \{ x \mid f(x) \neq g(x) \} \). Now
\[ x \in \{ x \in X \mid f(x) \neq g(x) \} \] implies \( f(x) \neq g(x) \) in \( Y \). By Hausdorff property there exists open set \( U_1 \) and \( U_2 \) such that \( f(x) \in U_1 \) and \( g(x) \in U_2 \) and \( U_1 \cap U_2 = \emptyset \).
...
Observe $f: M_n(C) \rightarrow C^{n^2}$ as homeomorphism.
$f(UC_n)$ is closed and bounded, and compact. This implies $UC_n$ is compact.
