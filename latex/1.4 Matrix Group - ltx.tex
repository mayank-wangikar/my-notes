...
\[
\tilde{f} : M_n(\mathbb{C}) \xrightarrow{} \mathbb{C}^{n^2}
\]
\[
(a_{ij}) \mapsto (b_{ij})
\]
...
where \( q: M_n(\mathbb{C}) \times M_n(\mathbb{C}) \rightarrow M_n(\mathbb{C}) \) is matrix multiplication. And \( p \) is a polynomial

\[
p((b_{ij}), (b'_{ij})) = (c_{ij}) \text{ where }
\]
...
Hence \( p \) is continuous. Therefore for \( q = p \circ f \times f \) is continuous. For any open set in \( M_n(\mathbb{C}) \) \( U \), we express \( U = f^{-1}(O) \) for some open set \( O \) in
\end{proof}
...
\item then \( ^t(\sigma^{-1}) = (^t \sigma)^{-1} \), \( (\bar{\sigma})^{-1} = (\sigma^{-1})^{-} \)
\end{enumerate}
\end{remark}
...
% --- Page 9 ---
\begin{definition}
$\mathrm{O}_n(\mathbb{C}) = \left\{ \sigma \in \mathrm{GL}_n(\mathbb{C}) \mid \sigma^* = \sigma \right\}$

is set of \underline{orthogonal matrices}. If $\sigma = \sigma^*$ then $\sigma$ is \underline{complex orthogonal} and if $\sigma = \sigma^* = \bar{\sigma}$ then real orthogonal.

$\mathrm{O}_n(\mathbb{C}) = \left\{ \sigma \in \mathrm{GL}_n(\mathbb{C}) \mid \sigma^* = \bar{\sigma} \right\}$

is set of all \underline{unitary matrices}.

Note $\mathrm{U}_n(\mathbb{C})$ or $\mathrm{O}_n(\mathbb{C})$ are non empty as $\varepsilon \in \mathrm{U}_n(\mathbb{C})$ and $\varepsilon \in \mathrm{O}_n(\mathbb{C})$.

Notation: $\mathrm{U}_n(\mathbb{C}) = (\mathrm{UC}_n)$ and $\mathrm{O}_n(\mathbb{R}) = \mathrm{OC}_n$.
...
Consider $ \{x \mid f(x) \neq g(x) \} $. Now
\[ x \in \{x \in X \mid f(x) \neq g(x) \} \] implies $f(x) \neq g(x)$
in $Y$. By Hausdorff property there exists open set $U_1$ and $U_2$ such that $f(x) \in U_1$ and $g(x) \in U_2$ and $U_1 \cap U_2 = \emptyset$. Now $f^{-1}(U_1)$ and $g^{-1}(U_2)$ are open. Let $V = f^{-1}(U_1) \cap g^{-1}(U_2)$ be a neighborhood of $x$. For any $y \in V$ $f(y) \neq g(y)$ otherwise $U_1 \cap U_2 \neq \emptyset$. Hence $x \in V \subseteq \{x \mid f(x) \neq g(x) \}$ is open. Hence $\{x \mid f(x) = g(x) \}$ is closed.
\end{proof}
...
The sets $U(n) = \{\sigma \mid \overline{\sigma} = \sigma^*\}$ and $O_n(\mathbb{C}) = \{\sigma \mid \sigma = \sigma^*\}$ are closed sets.
Observe $GL_n(\mathbb{R}) = \{\sigma \mid \overline{\sigma} = \sigma\}$ is also closed.
Hence $U(n)$ and $O(n) = O_n(\mathbb{C}) \cap GL_n(\mathbb{R})$ is closed.
...
As $\epsilon \in SL_n(\mathbb{C})$, and by subgroup tests we can show $SL_n(\mathbb{C})$ is a subgroup of $GL_n(\mathbb{C})$. Moreover, $SL_n(\mathbb{C}) = \det^{-1}\{1\}$ implies $SL_n(\mathbb{C})$ is a closed subgroup.
...
% --- Page 14 ---
implies $\sum_{k=1}^n |a_{ki}|^2 = 1$ which means for $i^{th}$ column vector, the absolute sum is 1. This implies for any $1 \leq i, j \leq n$ $|a_{ij}| \leq 1$.

Now in $(C^{n^2}, \|\cdot\|_1)$, $v = (v_k)_{k=1}^{n^2}$, $|v_k| \leq 1$ for $1 \leq k \leq n^2$ we define $\|v\|_1 = \max_{1 \leq k \leq n^2} |v_k|$. Observe $f: M_n(C) \rightarrow C^{n^2}$ as homeomorphism $f(UC_n)$ is closed and bounded, and compact. This implies $UC_n$ is compact.
