...
\begin{itemize}
    ...
    \begin{definition}\label{def:submodule_z}
        $N$ is a submodule iff $(N, +) \lessdot (CM, +)$.
    \end{definition}
...
\begin{itemize}
    ...
    \item \begin{remark}\label{rem:vector_fields}
        For vector fields, linear transformations $\leftrightarrow$ module homomorphisms.
    \end{remark}

    \item \begin{remark}\label{rem:ring_homomorphism}
        For a ring $R$, $\Theta: M \to N$ where $M = N = R$ and $\Theta(rs) = r\Theta(s)$ is \textbf{not} a ring homomorphism but is a module homomorphism.
    \end{remark}
