...
\begin{proof}
\begin{enumerate}[(i)]
\item Observe $\overline{H} = \bigcap_{V \in \mathcal{V}} HV$, where $\mathcal{V}$ is collection of all nbds of $e$. As $H \subseteq \overline{H}$ hence $\overline{H} \neq \emptyset$.
...
Hence $\overline{H} \times \overline{H} = \overline{H \times H} \subseteq m^{-1}(\overline{H})$ and this implies $m(\overline{H} \times \overline{H}) \subseteq \overline{H}$.
\end{enumerate}
\end{proof}
...
\begin{definition}
\underline{Set of all left cosets} set $G/H$. We have a natural map or projection
\[
...
Let $O$ be an open map in $G/H$ then $\pi(O)$ is open in $G/H$ iff $\pi^{-1}(\pi(O))$ is open in $G$. Now $OH = \pi^{-1}(\pi(O))$ and $OH = \bigcup Oh$. Hence $OH$ is open. So
\end{proof}
...
Now $x \in xH$ and $x \notin G \setminus yH$ which is open as $H$ is closed and $yH$ is closed. Now, let $V$ be a neighborhood of $x$ such that $x \in V \subset G \setminus yH$.

Observe $V \cap xH \cap yH = \emptyset$ implies $V \cap xH \cap yH = \emptyset$ otherwise $v h_1 = g h_2$ and $v h = g h_2 h_1^{-1}$ implying $V \cap xH \cap yH \neq \emptyset$. Now, we can find a neighborhood $V_1$ of $x$ such that $V_1^{-1} V_1 \subset V$ therefore $V_1^{-1} V_1 \cap xH \cap yH = \emptyset$.

This implies $V_1 xH \cap V_1 yH = \emptyset$, which is
$\pi(V_1 x) \cap \pi(V_1 y) = \emptyset$ and by the previous lemma $\pi(V_1 x)$ and $\pi(V_1 y)$ are open sets and $xH \in \pi(V_1 x)$ and $yH \in \pi(V_1 y)$. Hence
\end{proof}

% --- Page 6 ---
\begin{remark}
We call $G/H$ the quotient space where $H$ is a closed subgroup of $G$, has equipped with quotient topology.
\end{remark}
...
\[
(\alpha_1, y_1) \mapsto \alpha_1 y_1^{-1}
\]
is continuous. And $\varphi: G \times G \to G$ defined by
\[
(\alpha, y) \mapsto \alpha y^{-1}
\]
is continuous.
\end{proof}
...
\begin{lemma}\label{lem:subgroup_compactness}
...
\[ U = \pi^{-1}(U_i), \]
and let \( x \in G \) such that \( \pi(x) = q_i \). As \( G \) is locally compact we get a neighborhood \( \overline{O} \) of \( x \) such that
\[ \overline{O} \text{ is compact and } \overline{O} \subseteq U \]
by the proposition given below. Then we have
\[ \pi(\overline{O}) \subseteq \pi(U) = U_i. \]

If \( O_i = \pi(O) \) is a neighborhood of \( q_i \). And \( \pi(\overline{O}) \) is a continuous image of a compact set hence we get
\[ \pi(\overline{O}) \text{ is compact.} \]
Since \( G/H \) is \( T_2 \)-space \( \pi(\overline{O}) \) is closed which implies
...
Thus \( O_i \) is a neighborhood of \( q_i \) such that
\[ \overline{O_i} \text{ is compact.} \]
It follows \( G/H \) is locally compact.
...
\begin{proof}
By properties (5) and (6) we know that for every neighborhood $U$ of $e$ we have a symmetric neighborhood of $e$ $V$ such that $V^2 \subseteq U$. Now $x \in \overline{V} = \bigcap_{w \in V} wV$ where $V^2$ is the set of all neighborhoods of $e$, this implies $x \in \overline{V} \subseteq V^2 \subseteq U$.
\end{proof}
