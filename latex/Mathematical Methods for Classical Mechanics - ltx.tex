...
% FIX 1: Changed from \begin{definition} to \begin{claim} — in the notes this is
% labelled "Claim: S' is cpt", not a definition.
\begin{claim}\label{clm:s1_compact}
$S^1$ is compact.
\end{claim}
...
% FIX 2: Relabelled \begin{question} as \begin{exercise} to match the document's
% theorem environment vocabulary (no \question env defined in preamble).
\begin{exercise}
...
\begin{enumerate}
    ...
    % FIX 3: Original notes say "(NOT desirable)", not "(not differentiable)".
    \item A closed interval, derivative is defined one-sided for end points (not desirable).
\end{enumerate}
\end{remark}
...
% FIX 4: Removed the spurious \qed tombstone (□) that appeared after the derivation.
% A derivation is not a proof; the □ was misleading.
...
% --- Page 7 ---
% FIX 5: Changed \mathbb{Z}(0,-1)^\top to \{(0,-1)\}.
% The original notes write "Ker P_N = {(0,-1)}" — a singleton set, not an integer span.
\begin{remark}
$\ker P_N = \{(0,-1)\}$;
\end{remark}
...
\begin{itemize}
    ...
    \]
    % FIX 6: Expanded 4(k^4-1) to match the handwritten working: 16k^2 - 4k^4 - 4 >= 0
    \[
    \Delta_y = 16k^2 - 4k^4 - 4 \geq 0 \implies (\alpha_k, y_k) \text{ exist.}
    \]
\end{itemize}
\end{proof}
...
% FIX 7: Rewrote Definition 0.4 — "st:" was confusing shorthand. The map is P_N (or P_S).
\begin{definition}\label{def:chart_coord}
We call the map $P_N$ (or $P_S$) a \underline{chart/coordinate system}.
\end{definition}
...
\begin{definition}\label{def:atlas_gen}
...
% --- Page 21 ---
\begin{definition}\label{def:real_projective_space_1}
...
\begin{definition}\label{def:real_projective_space_n}
...
\begin{definition}\label{def:rp_quotient}
\[
\mathbb{RP}^n = S^n / \sim
\]
...
% --- Page 27 ---
---
% FIX 8: Resolved dangling cross-reference "??" — corrected to point to
% def:smooth_function_curve (Definition 0.29 in the compiled document).
\begin{definition}\label{def:differentiability_function}
Using Definition~\ref{def:smooth_function_curve}, we can define differentiability of function
\[ g: X \to \mathbb{R}^m. \]
...
% FIX 9: Changed "Problem 1: SO(3) is homeomorphic to RP^2" to RP^3.
% The handwritten notes and Theorem 0.1 above both confirm it is RP^3.
\begin{problem}
$SO(3)$ is homeomorphic to $\mathbb{RP}^3$.
\end{problem}
...
through $(=\gamma_1(0)=\gamma_2(0)=p)$ are \textbf{equivalent} for any chart if
...
Two locally defined curves $\gamma_1$ and $\gamma_2$ through a point $p$ are called \textbf{equivalent} if for any chart $\varphi$ around $p$,
\[
\left.\frac{d}{dt} (\varphi \circ \gamma_1)\right|_{t=0} = \left.\frac{d}{dt} (\varphi \circ \gamma_2)\right|_{t=0}.
\]
\end{definition}
...
% --- Page 35 ---
% FIX 10: Removed duplicate bullet. The original had ∂/∂xi|a(cf) = c ∂f/∂xi|a listed twice.
\[
...
\begin{definition}\label{def:tangent_bundle_chart}
...
% FIX 11: Corrected T\mathbb{R}^n to T^*\mathbb{R}^n (missing star on cotangent bundle).
...
\begin{definition}\label{def:exterior_2_form_R3}
...
% FIX 12: Removed dangling \ref{**} — replaced with prose reference.
\begin{exercise}
Show the dimension of $\Lambda^2(\mathbb{C} \mathbb{R}^n)$ is given by $\binom{n}{2}$. We first show the set $\left\{ dx_i \wedge dx_j \mid i < j \right\}$ forms a basis for $\Lambda^2(\mathbb{R}^n)$ (Show).
...
% --- Page 67 ---
\begin{definition}\label{def:differential_2_form_manifold}
...
In a chart $(U, \varphi = (x_1, \dots, x_n))$ on $M$, $\omega^2$ is expressed in $(\ast)$.
...
% --- Page 76 ---
\begin{definition}\label{def:kform_coords}
...
% FIX 13: Changed \begin{question} to \begin{exercise} (question env not defined in preamble).
\begin{exercise}
...
\begin{definition}\label{def:B_functional}
...
\begin{example}\label{ex:tangent_bundle_s1}
Let $X = S^1$.
...
% FIX 14: Replaced raw **PART 1** and **PART 2** markdown markers with proper LaTeX bold.
\textbf{PART 1} $dH$ (= Total derivative of $H$)

\[
dH = \frac{\partial H}{\partial q} \, dq + \frac{\partial H}{\partial p} \, dp
\]

\textbf{PART 2} Now find \( X_H \) such that
...
% --- Page 107 ---
\begin{definition}\label{def:vector_field_manifold}
...
Apply Cartan's Magic Formula to compute $\mathcal{L}_{y \frac{\partial}{\partial x}}$:

\[
\mathcal{L}_{y \frac{\partial}{\partial x}} (dx \wedge dy) = \mathrm{d} \left( y \frac{\partial}{\partial x} \lrcorner (dx \wedge dy) \right) + y \frac{\partial}{\partial x} \lrcorner \mathrm{d}(dx \wedge dy)
\]
